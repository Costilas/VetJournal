    ...
    <div class="note_wrapper">
        <div class="note create_note_status">
            <form method="POST" action="{{route('note.create')}}">
                ...
                <input class="form-control w-75" type="text" name="theme" placeholder="Тема заметки" value="{{old('body')}}">
                <textarea class="form-control" name="body" placeholder="Текст заметки">{{old('body')}}</textarea>
        ...
        @if(!empty($notes))
            @foreach($notes as $note)
                <div class="note note_item {{$note->status->class_name}}_status">
                    ...
                    <p class="status">{{$note->status->name}}</p>
